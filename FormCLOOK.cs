...
namespace OSProject
{
    public partial class FormCLOOK : Form
    {
        ...
        private void CalculateCLOOK(int head)
        {
            sequence.Clear();
            sequence.Add(head);
            ...
            {
                int firstLeft = remainingRequests.Min();
                totalSeek += Math.Abs(currentHead - firstLeft);
                currentHead = firstLeft;
                sequence.Add(firstLeft);
                remainingRequests.Remove(firstLeft);
            }
            // Move right from first left request
        ...
        private void DrawVisualization()
        {
            ...
            int maxTrack = Math.Max(sequence.Max(), 199);
